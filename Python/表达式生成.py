...
from ExpGen import 表达式 as 表达式pyd
...
class 非终结(): #非叶子节点
    def __init__(self, 原头节点 = None) -> None:
        ...
        if(右边先算):
           ...
           print("请自己琢磨这该咋写吧!")#当前先算时, 它的层级会下降, 会变成新表达式的左下子表达式, 所以当前的头节点会作为参数, 让新节点的左节点指向它
...
if __name__ == "__main__":
    ...
    表达式1 = 表达式pyd()
    print(表达式1)
    表达式1.树状图.render("表达式1.gv", view= True, format='png')
    正确 = False
    不正确计数 = 0
    for i in range(40):
        表 = 表达式pyd()
        表达式值 = eval(表.算式)  #把表达式字串给Python算, 看结果和通过深度遍历求得的值可一样
        误差 = abs(表.头节点.数值 - 表达式值)
        正确 = 误差 <= 5e-12  #由于表达式字串把一些不必要的括号去掉了, 导致计算顺序上的不同, 理论上不影响结果, 但实际上有微小的浮点计算误差
        if not 正确:
            不正确计数 += 1
            print("err " + str(误差))
            print(表达式值)
