...
#define 宽词打印码  CP_UTF8  //clang编译转为utf-8
#endif
...
public:
    ...
    void 保存双数组(std::string 文件名) {
        std::ofstream 输出流base(文件名 + ".base", std::ios::out | std::ios::binary);
        std::ofstream 输出流check(文件名 + ".check", std::ios::out | std::ios::binary);
        输出流base.write((char*)&base[0], base.size() * sizeof(base[0]));
        输出流check.write((char*)&next_check[0], next_check.size() * sizeof(next_check[0]));
        输出流base.close();
        输出流check.close();
        std::cout << "base: " << base.size() << " check: " << next_check.size() << std::endl;
    }

    void 加载双数组(std::string 文件名) {
        std::ifstream 输入流base(文件名 + ".base", std::ios::in | std::ios::binary);
        std::ifstream 输入流check(文件名 + ".check", std::ios::in | std::ios::binary);
        int base数据 = 0;
        while (输入流base.read((char*)&base数据, sizeof(base数据)))
            base.push_back(base数据);
        双数组 check数据;
        while (输入流check.read((char*)&check数据, sizeof(check数据)))
            next_check.push_back(check数据);
        输入流base.close();
        输入流check.close();
    }
...
void 字典树性能测试() {
    ...
    wchar_t 字;
    {
        ...
        for (size_t i = 0; i < 词表.size(); i++)
            for (size_t j = 0; j < 词表[i].size(); j++) {
                字 = 词表[i][j];
            }
    }
    std::wcout << 字 << std::endl;
    ...
    {
        计时器 计时("英词 数组树查询: ");
        for (size_t i = 0; i < 词表.size(); i++) //for(auto 词 : 词表)这样写会慢些
            结果 = 数组树.查词(词表[i]);
    }
    std::wcout << 结果.字母号 << std::endl;
    {
        计时器 计时("英词 双数组查询: ");
        for (size_t i = 0; i < 词表.size(); i++) //for(auto 词 : 词表)这样写会慢些
    ...
    auto 文本乱序词表 = 读入词库("长短en.txt");//字串存储顺序乱对查询耗时有影响, 但没有字典树中状态表的乱序查询影响大
    size_t 重复次数 = 10;
    {
        计时器 计时("英词 乱序数组树查询10次: ");
        for (size_t j = 0; j < 重复次数; j++)
            for (size_t i = 0; i < 文本乱序词表.size(); i++)
                结果 = 数组树.查词(文本乱序词表[i]);
    }
    std::wcout << 结果.字母号 << std::endl;
    {
        计时器 计时("英词 乱序双数组查询10次: ");
        for (size_t j = 0; j < 重复次数; j++)
            for (size_t i = 0; i < 文本乱序词表.size(); i++)
                auto 结果 = 数组树.双数组查词(文本乱序词表[i]);
    }
   ...
   /* 剪枝压缩字典树 剪枝树(横表哈希树);
    ...
    }
    {
        ...
        for (size_t j = 0; j < 重复次数; j++)
            for (size_t i = 0; i < 文本乱序词表.size(); i++)
                auto 结果 = 剪枝树.查词(文本乱序词表[i]);
    }
    {
        计时器 计时("英词 乱序剪枝双数组查询10次: ");
        for (size_t j = 0; j < 重复次数; j++)
            for (size_t i = 0; i < 文本乱序词表.size(); i++)
                auto 结果 = 剪枝树.双数组查词(文本乱序词表[i]);
    }
    std::cout << std::endl;*/

    ska::flat_hash_map<std::wstring, int> 字串哈希;
    for (size_t i = 0; i < 词表.size(); i++) {
        字串哈希[词表[i]] = i;
    }
    int 哈希结果 = 0;
    {
        计时器 计时("英词 乱序字串哈希表10次查询: "); //乱不乱序对查字串哈希没影响
        for (size_t j = 0; j < 重复次数; j++)
            for (size_t i = 0; i < 文本乱序词表.size(); i++)
                哈希结果 = 字串哈希[文本乱序词表[i]];
    }

    std::cout << 哈希结果 <<std::endl;

    //auto 中词表 = 读入词库("cn.txt"); //中文只能用哈希表字典树, 否则状态表数组会太大
    //{
    //    计时器 计时("读一遍字: ");
    //    for (size_t i = 0; i < 中词表.size(); i++)
    //        for (size_t j = 0; j < 中词表[i].size(); j++) {
    //            字 = 中词表[i][j];
    //        }
    //}
    //std::wcout << 字 << std::endl;
    //{
    //    横表字典树<横表哈希节点> 哈希树(中词表, 中文); //中文词短, 查起来比英文快
    //    //哈希树.打印树信息();
    //    计时器 计时("中词 哈希树查询: ");
    //    for (size_t i = 0; i < 中词表.size(); i++)
    //        结果 = 哈希树.查词(中词表[i]);
    ...
    //{
    //    ska::flat_hash_map<std::wstring, int> 字串哈希;
    //    for (size_t i = 0; i < 中词表.size(); i++) 字串哈希[中词表[i]] = i + 1;
    //    计时器 计时("中词 字串哈希表查询: ");
    //    for (size_t i = 0; i < 中词表.size(); i++)
    //        auto 结果 = 字串哈希[中词表[i]];
    //}
    //std::cout << std::endl;
    //for (auto 词 : 中词表) 词表.push_back(词);// 英文 中文词库放一起. 
    //
    //{
    //    计时器 计时("读一遍字: ");
    //    for (size_t i = 0; i < 词表.size(); i++)
    ...
    //{
    //    ska::flat_hash_map<std::wstring, int> 字串哈希;
    //    for (size_t i = 0; i < 词表.size(); i++) 字串哈希[词表[i]] = i + 1;
    //    计时器 计时("中英 字串哈希表查询: ");
    //    for (size_t i = 0; i < 词表.size(); i++)
    //        auto 结果 = 字串哈希[词表[i]];
    //}
