...
public class 文件 {
    ...
    static public void 保存图片(Bitmap 位图, string 文件名, ImageFormat 图片格式, bool 二值 = false) {
        Bitmap 待存位图 = 二值 ? 位图.Clone(new Rectangle(0, 0, 位图.Width, 位图.Height), PixelFormat.Format1bppIndexed) : 位图;
        try {
            待存位图.Save(文件名, 图片格式);
        } catch {
            using var 位图拷贝 = new Bitmap(待存位图);
            位图拷贝.Save(文件名, 图片格式);
        }
    }

    static public ImageFormat 根据文件格式保存图片(Bitmap 位图, string 文件名, bool 二值 = false) {
        ImageFormat 图片格式;
